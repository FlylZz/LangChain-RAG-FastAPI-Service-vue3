...
def load_system_prompt():
    ...
    try:
        ...
        logger.error(f"[load_system_prompt]读取系统提示词文件出错：{e}")
        raise e
...
def load_rag_summarize_prompt():
    try:
        ...
        logger.error(f"[load_rag_summarize_prompt]在yaml配置项中没有rag_summarize_prompt_path配置项")
        raise e

    try:
        return open(rag_summarize_prompt_path, "r", encoding="utf-8").read()
    except Exception as e:
        logger.error(f"[load_rag_summarize_prompt]读取RAG总结提示词文件出错：{e}")
        raise e
...
def load_report_prompt():
    ...
    try:
        ...
        logger.error(f"[load_report_prompt]读取报告提示词文件出错：{e}")
        raise e
